...
comment on type public.user_role is
  'Sebetsa application roles. Mirrors src/features/auth/types/auth.types.ts — keep both in sync.';

alter table public.profiles add column role public.user_role;

comment on column public.profiles.role is
  'Denormalized mirror of the JWT app_metadata.role claim — NOT the authorization source of truth. Written only by admin_update_user_role()/admin_create_user().';

create index profiles_role_idx on public.profiles (role);
create index profiles_tenant_id_role_idx on public.profiles (tenant_id, role);

-- Write access: an organization_administrator (or platform admin) may edit
-- any profile's contact info/status within their own tenant.
...
comment on function public.can_manage_profiles(uuid) is
  'True for organization_administrator/operations_manager/hr_user of the target tenant, or any platform admin.';
...
-- admin_update_user_role() immediately before its UPDATE) may modify `role`.
...
-- Role assignment: platform admins are unrestricted; an
-- organization_administrator may assign any non-platform role within their
-- own tenant. No one else may assign roles in this MVP.
...
as $$
  select case
    ...
    when coalesce(auth.jwt() -> 'app_metadata' ->> 'role', '') = 'organization_administrator' then
      p_new_role <> 'platform_administrator'
    else false
  end
$$;
...
-- admin_create_user: creates auth.users + auth.identities + the matching
-- profile in one SECURITY DEFINER call. Returns a one-time temporary
-- password; no transactional email sending is configured yet.
